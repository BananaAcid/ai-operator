...
export type KeypressHandler = ({
  key, rl,
  config, theme, firstRender, status, setStatus, prefix, searchResults, active, setActive
}:{
  key: KeypressEvent & {meta?:boolean, sequence?:string, shift?:boolean}, rl: InquirerReadline
  config: SearchConfig<any>, theme: any, firstRender: any, status: any, setStatus: any, prefix: any, searchResults: any|void, active: any, setActive: any
...
export default createPrompt(
  <Value>(config: SearchConfig<Value>, done: (value: Value) => void) => {
    ...
    let selectedChoice = searchResults[active] as NormalizedChoice<Value> | void;
    ...
    useKeypress(async (key, rl) => {
      ...
      if (config.keypressHandler) {
          ...
          if (act && act?.needRefresh) {
              selectedChoice = searchResults[active] as NormalizedChoice<Value>; // since there is no setItems, ref needs refresh
